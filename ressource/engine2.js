    ...
    this.paintCanvas  = function() {
        ...
        for (y = 0; y < objConsts.playFieldSize(); y++) {
            if(this.coordold[y] !== this.coord[y]){
                $("#img"+y).html("<img src='../images/" + (this.coord[y] >= 30 ? this.coord[y] - 30 : this.coord[y]) + ".png' height='32' width='31' />");
            }
        }
        for (y = 0; y < objConsts.playFieldSize(); y++) {
...
var objTimer = {
    start: function(interval , game) {
        if (game.id === "1"){
            if (player1.tMainLoop !== undefined) {
                clearInterval(player1.tMainLoop);
            }
            player1.tMainLoop = setInterval(player1.mainLoop(), interval);
            console.log(player1.tMainLoop);
            player1.tRemove = setInterval(player1.clearMarbles(), interval);
        }else{
            if (player2.tMainLoop !== undefined) {
                clearInterval(player2.tMainLoop);
            }
            player2.tMainLoop = setInterval(player2.mainLoop(), interval);
            console.log(player1.tMainLoop);
            player2.tRemove = setInterval(player2.clearMarbles(), interval);
        }
    },
    stop: function(game) {
